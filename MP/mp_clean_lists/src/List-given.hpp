...
void List<T>::sort() {
    ...
    tail_ = head_;
    while (tail_->next != NULL) tail_ = tail_->next;
}
...
template <class T>
List<T>::ListNode::ListNode() : next(nullptr), prev(nullptr), data(T())
{ /* nothing */ }

template <class T>
List<T>::ListNode::ListNode(const T& ndata) : next(nullptr), prev(nullptr), data(ndata)
{ /* nothing */ }
