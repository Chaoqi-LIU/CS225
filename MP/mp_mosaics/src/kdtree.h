...
class KDTree
{
  ...
  public:
    ...
    vector<Point<Dim>> GetPoints() { return points_; }
  ...
  private:
    ...
    void Swap(Point<Dim> & pt1, Point<Dim> & pt2) {
      Point<Dim> tmp = pt1;
      pt1 = pt2;
      pt2 = tmp;
    }

    int partition(int dim, int l, int r) {
      int i = (l + r) / 2;
      Swap(points_.at(i), points_.at(r));
      Point x = points_.at(r);
      i = l - 1;
      for (int j = l; j < r; ++j)
        if (smallerDimVal(points_.at(j), x, dim)) Swap(points_.at(++i), points_.at(j));
      Swap(points_.at(++i), points_.at(r));
      return i;
    }

    Point<Dim> quick_select(int dim, int l, int r, int k) {
      if (l > r) return Point<Dim>();
      int pos = partition(dim, l, r);
      if (pos == k) return points_.at(k);
      if (pos > k) return quick_select(dim, l, pos - 1, k);
      else return quick_select(dim, pos + 1, r, k);
    }

    KDTreeNode * BuildKDTree(int dim, int l, int r) {
      if (l > r) return nullptr;
      KDTreeNode * node = new KDTreeNode(quick_select(dim, l, r, (l + r) / 2));
      node->left = BuildKDTree((dim+1)%Dim, l, (l+r)/2 - 1);
      node->right = BuildKDTree((dim+1)%Dim, (l+r)/2 + 1, r);
      return node;
    }
    ...
    void findNearestNeighbor(KDTreeNode * node, const Point<Dim>& query, int dim, Point<Dim>& best, double & best_dis) const {
      if (node == nullptr) return;

      if (node->left == nullptr && node->right == nullptr) {
        if (shouldReplace(query, best, node->point)) {
          best = node->point;
          best_dis = EuclideanDis(best, query);
          return;
        }
      }
      ...
      // go left
      if (smallerDimVal(query, node->point, dim) && node->left != nullptr) {
        findNearestNeighbor(node->left, query, (dim+1)%Dim, best, best_dis);
        should_replace = shouldReplace(query, best, node->point);
        if (should_replace || HasIntersection(best_dis, query, node->point, dim)) {
          if (should_replace) best = node->point, best_dis = EuclideanDis(best, query);
          findNearestNeighbor(node->right, query, (dim+1)%Dim, best, best_dis);
        }
      // go right
      } else {
        findNearestNeighbor(node->right, query, (dim+1)%Dim, best, best_dis);
    ...
    double EuclideanDis(const Point<Dim> & a, const Point<Dim> &  b) const {
      double dis = 0;
      for (int i = 0; i < Dim; ++i) {
        dis += pow( (a[i] - b[i]), 2);
      }
    ...
    bool HasIntersection(double best_dis, const Point<Dim> & target, const Point<Dim> & potential, int dim) const {
      return best_dis >= pow( (potential[dim] - target[dim]), 2) ;
    }
