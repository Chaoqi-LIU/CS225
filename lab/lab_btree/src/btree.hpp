...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    if (subroot->is_leaf) {
        if (first_larger_idx < subroot->elements.size() && 
            subroot->elements.at(first_larger_idx).key == pair.key) return;
        subroot->elements.insert(subroot->elements.begin() + first_larger_idx, pair);
    } else {
        insert(subroot->children.at(first_larger_idx), pair);
    }

    for (size_t child_idx = 0; child_idx < subroot->children.size(); ++child_idx) {
        if (subroot->children.at(child_idx)->elements.size() == order) {
            split_child(subroot, child_idx);
            break;
        }
    }
}
