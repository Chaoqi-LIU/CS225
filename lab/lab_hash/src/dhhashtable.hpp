...
void DHHashTable<K, V>::insert(K const& key, V const& value)
{
    ...
    unsigned int loc = hashes::hash(key, size),
                 jump = hashes::secondary_hash(key, size);
    while (should_probe[loc]) loc = (loc + jump) % size;
...
int DHHashTable<K, V>::findIndex(const K& key) const
{
    ...
    unsigned int loc = hashes::hash(key, size),
                 h2 = hashes::secondary_hash(key, size);
    while (should_probe[loc] && table[loc]->first != key) loc = (loc + h2) % size;
