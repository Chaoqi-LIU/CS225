                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if (singleQueue.empty()) to_return = mergeQueue.front(), mergeQueue.pop();
    else if (mergeQueue.empty()) to_return = singleQueue.front(), singleQueue.pop();
    else if (singleQueue.front()->freq < mergeQueue.front()->freq) to_return = singleQueue.front(), singleQueue.pop();
    else to_return = mergeQueue.front(), mergeQueue.pop();
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    for (auto & frequency : frequencies) singleQueue.push(new TreeNode(frequency));

    TreeNode * left, * right, * parent;
    while ( ! ((singleQueue.empty() && mergeQueue.size() == 1) 
            || (singleQueue.size() == 1 && mergeQueue.empty()))) {
        left = removeSmallest(singleQueue, mergeQueue);
        right = removeSmallest(singleQueue, mergeQueue);
        parent = new TreeNode(left->freq.getFrequency() + right->freq.getFrequency());
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if (bfile.hasBits()) {
        if (bfile.getNextBit() == 1) created = new TreeNode(Frequency(bfile.getNextByte(), 0));
        else {
            created = new TreeNode(Frequency('\0', 0));
            created->left = readTree(bfile);
            created->right = readTree(bfile);
        }
    }
